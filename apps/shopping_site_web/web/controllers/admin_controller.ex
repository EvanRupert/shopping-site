defmodule ShoppingSiteWeb.AdminController do
  ...
  def create(conn, %{
        "item" => %{"description" => des, "name" => name, "image" => upload, "price" => price}
      }) do
    ...
    case ItemQueries.insert_item(name, des, p, upload.filename) do
      {:error, reason} ->
        ...
        render(conn, "admin.html", items: items, errors: reason.errors, changeset: changeset)
  ...
  def create(conn, %{"item" => %{"description" => des, "name" => name, "price" => price}}) do
    {p, _} = Float.parse(price)
  ...
  def edit(conn, %{
        "submit" => "edit",
        "item" => %{"id" => id, "name" => name, "description" => des, "price" => price}
      }) do
  ...
  def edit(conn, %{"submit" => "delete", "item" => %{"id" => id}}) do
    ...
    ShoppingSite.ItemQueries.delete_by_id(
      num
      |> IO.inspect()
    )
